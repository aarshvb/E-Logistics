package org.apache.jsp;

import javax.servlet.*;
import javax.servlet.http.*;
import javax.servlet.jsp.*;
import Logic.Function;
import java.sql.Connection;
import bean.workorderBean;

public final class vendorgrid_jsp extends org.apache.jasper.runtime.HttpJspBase
    implements org.apache.jasper.runtime.JspSourceDependent {

  private static final JspFactory _jspxFactory = JspFactory.getDefaultFactory();

  private static java.util.List<String> _jspx_dependants;

  private org.glassfish.jsp.api.ResourceInjector _jspx_resourceInjector;

  public java.util.List<String> getDependants() {
    return _jspx_dependants;
  }

  public void _jspService(HttpServletRequest request, HttpServletResponse response)
        throws java.io.IOException, ServletException {

    PageContext pageContext = null;
    HttpSession session = null;
    ServletContext application = null;
    ServletConfig config = null;
    JspWriter out = null;
    Object page = this;
    JspWriter _jspx_out = null;
    PageContext _jspx_page_context = null;

    try {
      response.setContentType("text/html;charset=UTF-8");
      pageContext = _jspxFactory.getPageContext(this, request, response,
      			null, true, 8192, true);
      _jspx_page_context = pageContext;
      application = pageContext.getServletContext();
      config = pageContext.getServletConfig();
      session = pageContext.getSession();
      out = pageContext.getOut();
      _jspx_out = out;
      _jspx_resourceInjector = (org.glassfish.jsp.api.ResourceInjector) application.getAttribute("com.sun.appserv.jsp.resource.injector");

      out.write("\n");
      out.write("\n");
      out.write("\n");
      out.write("\n");
      out.write("\n");
      out.write("\n");
      out.write("\n");
      out.write("<html>\n");
      out.write("  <head>\n");
      out.write("    <meta charset=\"utf-8\">\n");
      out.write("    <meta http-equiv=\"X-UA-Compatible\" content=\"IE=edge\">\n");
      out.write("    <title>AdminLTE 2 | General Form Elements</title>\n");
      out.write("    <!-- Tell the browser to be responsive to screen width -->\n");
      out.write("    <meta content=\"width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no\" name=\"viewport\">\n");
      out.write("    <!-- Bootstrap 3.3.5 -->\n");
      out.write("    <link rel=\"stylesheet\" href=\"admincss/bootstrap/css/bootstrap.min.css\">\n");
      out.write("    <!-- Font Awesome -->\n");
      out.write("    <link rel=\"stylesheet\" href=\"https://maxcdn.bootstrapcdn.com/font-awesome/4.4.0/css/font-awesome.min.css\">\n");
      out.write("    <!-- Ionicons -->\n");
      out.write("    <link rel=\"stylesheet\" href=\"https://code.ionicframework.com/ionicons/2.0.1/css/ionicons.min.css\">\n");
      out.write("    <!-- Theme style -->\n");
      out.write("    <link rel=\"stylesheet\" href=\"admincss/dist/css/AdminLTE.min.css\">\n");
      out.write("    <!-- AdminLTE Skins. Choose a skin from the css/skins\n");
      out.write("         folder instead of downloading all of them to reduce the load. -->\n");
      out.write("    <link rel=\"stylesheet\" href=\"admincss/dist/css/skins/_all-skins.min.css\">\n");
      out.write("\n");
      out.write("    <!-- HTML5 Shim and Respond.js IE8 support of HTML5 elements and media queries -->\n");
      out.write("    <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->\n");
      out.write("    <!--[if lt IE 9]>\n");
      out.write("        <script src=\"https://oss.maxcdn.com/html5shiv/3.7.3/html5shiv.min.js\"></script>\n");
      out.write("        <script src=\"https://oss.maxcdn.com/respond/1.4.2/respond.min.js\"></script>\n");
      out.write("    <![endif]-->\n");
      out.write("    ");

        Connection con=Function.getConnection();
     
        workorderBean wb =new workorderBean();
        String workkey=wb.workorderkey(con);
        
    
      out.write("\n");
      out.write("\n");
      out.write("    <script lang=\"javascript\">\n");
      out.write("        \n");
      out.write("        function CreateItemTable()\n");
      out.write("        {\n");
      out.write("            var table = document.getElementById(\"item\");\n");
      out.write("            \n");
      out.write("            var noofrow=table.rows.length;\n");
      out.write("            alert(noofrow);\n");
      out.write("            var tbody;\n");
      out.write("            //table.innerHTML=\"\";\n");
      out.write("            if(noofrow==0)\n");
      out.write("            {\n");
      out.write("            var thead = document.createElement(\"thead\");\n");
      out.write("            var row = thead.insertRow(0);   \n");
      out.write("            var th = document.createElement(\"th\");\n");
      out.write("            th.innerHTML = \"Check\";\n");
      out.write("            row.appendChild(th);\n");
      out.write("\n");
      out.write("            var th1 = document.createElement(\"th\");\n");
      out.write("            th1.innerHTML = \"Iteam Name\";\n");
      out.write("            row.appendChild(th1);\n");
      out.write("\n");
      out.write("            var th2 = document.createElement(\"th\"); \n");
      out.write("            th2.innerHTML = \"Quantity\";\n");
      out.write("            row.appendChild(th2);\n");
      out.write("\n");
      out.write("\n");
      out.write("            var th3 = document.createElement(\"th\");\n");
      out.write("            th3.innerHTML = \"unit\";\n");
      out.write("            row.appendChild(th3);\n");
      out.write("\n");
      out.write("            table.appendChild(thead);\n");
      out.write("            \n");
      out.write("            tbody = document.createElement(\"tbody\");\n");
      out.write("            tbody.setAttribute(\"id\",\"itembody\");\n");
      out.write("            table.appendChild(tbody);\n");
      out.write("        }\n");
      out.write("        //else\n");
      out.write("        //{\n");
      out.write("            tbody=document.getElementById(\"itembody\");\n");
      out.write("            var row = tbody.insertRow(noofrow-1);\n");
      out.write("            var cell1 = row.insertCell(0);\n");
      out.write("              \n");
      out.write("            var element6 = document.createElement(\"input\");\n");
      out.write("            element6.setAttribute(\"type\", \"checkbox\");\n");
      out.write("            element6.setAttribute(\"name\", \"chk\"+noofrow);\n");
      out.write("           // element6.setAttribute(\"value\", val[0]);\n");
      out.write("            cell1.appendChild(element6);//checkbox\n");
      out.write("            \n");
      out.write("             var cell3 = row.insertCell(1);\n");
      out.write("               \n");
      out.write("             var element2 = document.createElement(\"input\");\n");
      out.write("             element2.setAttribute(\"type\", \"textbox\");\n");
      out.write("             element2.setAttribute(\"value\", \"\");\n");
      out.write("             element2.setAttribute(\"name\", \"item\"+noofrow);\n");
      out.write("             cell3.appendChild(element2);\n");
      out.write("             \n");
      out.write("            var cell4 = row.insertCell(2);\n");
      out.write("               \n");
      out.write("             var element3 = document.createElement(\"input\");\n");
      out.write("             element3.setAttribute(\"type\", \"textbox\");\n");
      out.write("             element3.setAttribute(\"value\", \"\");\n");
      out.write("             element3.setAttribute(\"name\", \"qty\"+noofrow);\n");
      out.write("             cell4.appendChild(element3);\n");
      out.write("             \n");
      out.write("            var cell5 = row.insertCell(3);\n");
      out.write("               \n");
      out.write("            var element=document.createElement(\"select\");\n");
      out.write("            element.setAttribute(\"name\",\"unit\"+noofrow);\n");
      out.write("            var option=document.createElement(\"option\");\n");
      out.write("            option.setAttribute(\"value\",\"kg\");\n");
      out.write("            option.innerHTML=\"kg\";\n");
      out.write("            element.appendChild(option);\n");
      out.write("            \n");
      out.write("            var option=document.createElement(\"option\");\n");
      out.write("            option.setAttribute(\"value\",\"pound\");\n");
      out.write("            option.innerHTML=\"pound\";\n");
      out.write("            element.appendChild(option);\n");
      out.write("            \n");
      out.write("            \n");
      out.write("            cell5.appendChild(element);\n");
      out.write("            \n");
      out.write("           \n");
      out.write("             \n");
      out.write("        //}                     \n");
      out.write("        table.appendChild(tbody);\n");
      out.write("\n");
      out.write("        }\n");
      out.write("    </script>\n");
      out.write("  </head>\n");
      out.write("  <body class=\"hold-transition skin-blue sidebar-mini\" onload=\"getUserType();\" >\n");
      out.write("    <div class=\"wrapper\">\n");
      out.write("        ");
      org.apache.jasper.runtime.JspRuntimeLibrary.include(request, response, "headercss.jsp", out, false);
      out.write("\n");
      out.write("   <!-- Left side column. contains the logo and sidebar -->\n");
      out.write("      <aside class=\"main-sidebar\">\n");
      out.write("        <!-- sidebar: style can be found in sidebar.less -->\n");
      out.write("        <section class=\"sidebar\">\n");
      out.write("          <!-- Sidebar user panel -->\n");
      out.write("          <div class=\"user-panel\">\n");
      out.write("            <div class=\"pull-left image\">\n");
      out.write("              <img src=\"admincss/dist/img/user2-160x160.jpg\" class=\"img-circle\" alt=\"User Image\">\n");
      out.write("            </div>\n");
      out.write("            <div class=\"pull-left info\">\n");
      out.write("              <p>Alexander Pierce</p>\n");
      out.write("              <a href=\"#\"><i class=\"fa fa-circle text-success\"></i> Online</a>\n");
      out.write("            </div>\n");
      out.write("          </div>\n");
      out.write("          <!-- search form -->\n");
      out.write("          <form method=\"get\" class=\"sidebar-form\" >\n");
      out.write("            <div class=\"input-group\">\n");
      out.write("              <input type=\"text\" name=\"q\" class=\"form-control\" placeholder=\"Search...\">\n");
      out.write("              <span class=\"input-group-btn\">\n");
      out.write("                <button type=\"submit\" name=\"search\" id=\"search-btn\" class=\"btn btn-flat\"><i class=\"fa fa-search\"></i></button>\n");
      out.write("              </span>\n");
      out.write("            </div>\n");
      out.write("          </form>\n");
      out.write("          <!-- /.search form -->\n");
      out.write("          <!-- sidebar menu: : style can be found in sidebar.less -->\n");
      out.write("          <ul class=\"sidebar-menu\">\n");
      out.write("            <li class=\"header\">MAIN NAVIGATION</li>\n");
      out.write("            <li class=\"treeview\">\n");
      out.write("              <a href=\"#\">\n");
      out.write("                <i class=\"fa fa-dashboard\"></i> <span>Dashboard</span> <i class=\"fa fa-angle-left pull-right\"></i>\n");
      out.write("              </a>\n");
      out.write("              <ul class=\"treeview-menu\">\n");
      out.write("                <li><a href=\"../../index.html\"><i class=\"fa fa-circle-o\"></i> Dashboard v1</a></li>\n");
      out.write("                <li><a href=\"../../index2.html\"><i class=\"fa fa-circle-o\"></i> Dashboard v2</a></li>\n");
      out.write("              </ul>\n");
      out.write("            </li>\n");
      out.write("            <li class=\"treeview\">\n");
      out.write("              <a href=\"#\">\n");
      out.write("                <i class=\"fa fa-files-o\"></i>\n");
      out.write("                <span>Layout Options</span>\n");
      out.write("                <span class=\"label label-primary pull-right\">4</span>\n");
      out.write("              </a>\n");
      out.write("              <ul class=\"treeview-menu\">\n");
      out.write("                <li><a href=\"../layout/top-nav.html\"><i class=\"fa fa-circle-o\"></i> Top Navigation</a></li>\n");
      out.write("                <li><a href=\"../layout/boxed.html\"><i class=\"fa fa-circle-o\"></i> Boxed</a></li>\n");
      out.write("                <li><a href=\"../layout/fixed.html\"><i class=\"fa fa-circle-o\"></i> Fixed</a></li>\n");
      out.write("                <li><a href=\"../layout/collapsed-sidebar.html\"><i class=\"fa fa-circle-o\"></i> Collapsed Sidebar</a></li>\n");
      out.write("              </ul>\n");
      out.write("            </li>\n");
      out.write("            <li>\n");
      out.write("              <a href=\"../widgets.html\">\n");
      out.write("                <i class=\"fa fa-th\"></i> <span>Widgets</span> <small class=\"label pull-right bg-green\">new</small>\n");
      out.write("              </a>\n");
      out.write("            </li>\n");
      out.write("            <li class=\"treeview\">\n");
      out.write("              <a href=\"#\">\n");
      out.write("                <i class=\"fa fa-pie-chart\"></i>\n");
      out.write("                <span>Charts</span>\n");
      out.write("                <i class=\"fa fa-angle-left pull-right\"></i>\n");
      out.write("              </a>\n");
      out.write("              <ul class=\"treeview-menu\">\n");
      out.write("                <li><a href=\"../charts/chartjs.html\"><i class=\"fa fa-circle-o\"></i> ChartJS</a></li>\n");
      out.write("                <li><a href=\"../charts/morris.html\"><i class=\"fa fa-circle-o\"></i> Morris</a></li>\n");
      out.write("                <li><a href=\"../charts/flot.html\"><i class=\"fa fa-circle-o\"></i> Flot</a></li>\n");
      out.write("                <li><a href=\"../charts/inline.html\"><i class=\"fa fa-circle-o\"></i> Inline charts</a></li>\n");
      out.write("              </ul>\n");
      out.write("            </li>\n");
      out.write("            <li class=\"treeview\">\n");
      out.write("              <a href=\"#\">\n");
      out.write("                <i class=\"fa fa-laptop\"></i>\n");
      out.write("                <span>UI Elements</span>\n");
      out.write("                <i class=\"fa fa-angle-left pull-right\"></i>\n");
      out.write("              </a>\n");
      out.write("              <ul class=\"treeview-menu\">\n");
      out.write("                <li><a href=\"../UI/general.html\"><i class=\"fa fa-circle-o\"></i> General</a></li>\n");
      out.write("                <li><a href=\"../UI/icons.html\"><i class=\"fa fa-circle-o\"></i> Icons</a></li>\n");
      out.write("                <li><a href=\"../UI/buttons.html\"><i class=\"fa fa-circle-o\"></i> Buttons</a></li>\n");
      out.write("                <li><a href=\"../UI/sliders.html\"><i class=\"fa fa-circle-o\"></i> Sliders</a></li>\n");
      out.write("                <li><a href=\"../UI/timeline.html\"><i class=\"fa fa-circle-o\"></i> Timeline</a></li>\n");
      out.write("                <li><a href=\"../UI/modals.html\"><i class=\"fa fa-circle-o\"></i> Modals</a></li>\n");
      out.write("              </ul>\n");
      out.write("            </li>\n");
      out.write("            <li class=\"treeview active\">\n");
      out.write("              <a href=\"#\">\n");
      out.write("                <i class=\"fa fa-edit\"></i> <span>Forms</span>\n");
      out.write("                <i class=\"fa fa-angle-left pull-right\"></i>\n");
      out.write("              </a>\n");
      out.write("              <ul class=\"treeview-menu\">\n");
      out.write("                <li class=\"active\"><a href=\"general.html\"><i class=\"fa fa-circle-o\"></i> General Elements</a></li>\n");
      out.write("                <li><a href=\"advanced.html\"><i class=\"fa fa-circle-o\"></i> Advanced Elements</a></li>\n");
      out.write("                <li><a href=\"editors.html\"><i class=\"fa fa-circle-o\"></i> Editors</a></li>\n");
      out.write("              </ul>\n");
      out.write("            </li>\n");
      out.write("            <li class=\"treeview\">\n");
      out.write("              <a href=\"#\">\n");
      out.write("                <i class=\"fa fa-table\"></i> <span>Tables</span>\n");
      out.write("                <i class=\"fa fa-angle-left pull-right\"></i>\n");
      out.write("              </a>\n");
      out.write("              <ul class=\"treeview-menu\">\n");
      out.write("                <li><a href=\"../tables/simple.html\"><i class=\"fa fa-circle-o\"></i> Simple tables</a></li>\n");
      out.write("                <li><a href=\"../tables/data.html\"><i class=\"fa fa-circle-o\"></i> Data tables</a></li>\n");
      out.write("              </ul>\n");
      out.write("            </li>\n");
      out.write("            <li>\n");
      out.write("              <a href=\"../calendar.html\">\n");
      out.write("                <i class=\"fa fa-calendar\"></i> <span>Calendar</span>\n");
      out.write("                <small class=\"label pull-right bg-red\">3</small>\n");
      out.write("              </a>\n");
      out.write("            </li>\n");
      out.write("            <li>\n");
      out.write("              <a href=\"../mailbox/mailbox.html\">\n");
      out.write("                <i class=\"fa fa-envelope\"></i> <span>Mailbox</span>\n");
      out.write("                <small class=\"label pull-right bg-yellow\">12</small>\n");
      out.write("              </a>\n");
      out.write("            </li>\n");
      out.write("            <li class=\"treeview\">\n");
      out.write("              <a href=\"#\">\n");
      out.write("                <i class=\"fa fa-folder\"></i> <span>Examples</span>\n");
      out.write("                <i class=\"fa fa-angle-left pull-right\"></i>\n");
      out.write("              </a>\n");
      out.write("              <ul class=\"treeview-menu\">\n");
      out.write("                <li><a href=\"../examples/invoice.html\"><i class=\"fa fa-circle-o\"></i> Invoice</a></li>\n");
      out.write("                <li><a href=\"../examples/profile.html\"><i class=\"fa fa-circle-o\"></i> Profile</a></li>\n");
      out.write("                <li><a href=\"../examples/login.html\"><i class=\"fa fa-circle-o\"></i> Login</a></li>\n");
      out.write("                <li><a href=\"../examples/register.html\"><i class=\"fa fa-circle-o\"></i> Register</a></li>\n");
      out.write("                <li><a href=\"../examples/lockscreen.html\"><i class=\"fa fa-circle-o\"></i> Lockscreen</a></li>\n");
      out.write("                <li><a href=\"../examples/404.html\"><i class=\"fa fa-circle-o\"></i> 404 Error</a></li>\n");
      out.write("                <li><a href=\"../examples/500.html\"><i class=\"fa fa-circle-o\"></i> 500 Error</a></li>\n");
      out.write("                <li><a href=\"../examples/blank.html\"><i class=\"fa fa-circle-o\"></i> Blank Page</a></li>\n");
      out.write("              </ul>\n");
      out.write("            </li>\n");
      out.write("            <li class=\"treeview\">\n");
      out.write("              <a href=\"#\">\n");
      out.write("                <i class=\"fa fa-share\"></i> <span>Multilevel</span>\n");
      out.write("                <i class=\"fa fa-angle-left pull-right\"></i>\n");
      out.write("              </a>\n");
      out.write("              <ul class=\"treeview-menu\">\n");
      out.write("                <li><a href=\"#\"><i class=\"fa fa-circle-o\"></i> Level One</a></li>\n");
      out.write("                <li>\n");
      out.write("                  <a href=\"#\"><i class=\"fa fa-circle-o\"></i> Level One <i class=\"fa fa-angle-left pull-right\"></i></a>\n");
      out.write("                  <ul class=\"treeview-menu\">\n");
      out.write("                    <li><a href=\"#\"><i class=\"fa fa-circle-o\"></i> Level Two</a></li>\n");
      out.write("                    <li>\n");
      out.write("                      <a href=\"#\"><i class=\"fa fa-circle-o\"></i> Level Two <i class=\"fa fa-angle-left pull-right\"></i></a>\n");
      out.write("                      <ul class=\"treeview-menu\">\n");
      out.write("                        <li><a href=\"#\"><i class=\"fa fa-circle-o\"></i> Level Three</a></li>\n");
      out.write("                        <li><a href=\"#\"><i class=\"fa fa-circle-o\"></i> Level Three</a></li>\n");
      out.write("                      </ul>\n");
      out.write("                    </li>\n");
      out.write("                  </ul>\n");
      out.write("                </li>\n");
      out.write("                <li><a href=\"#\"><i class=\"fa fa-circle-o\"></i> Level One</a></li>\n");
      out.write("              </ul>\n");
      out.write("            </li>\n");
      out.write("            <li><a href=\"../../documentation/index.html\"><i class=\"fa fa-book\"></i> <span>Documentation</span></a></li>\n");
      out.write("            <li class=\"header\">LABELS</li>\n");
      out.write("            <li><a href=\"#\"><i class=\"fa fa-circle-o text-red\"></i> <span>Important</span></a></li>\n");
      out.write("            <li><a href=\"#\"><i class=\"fa fa-circle-o text-yellow\"></i> <span>Warning</span></a></li>\n");
      out.write("            <li><a href=\"#\"><i class=\"fa fa-circle-o text-aqua\"></i> <span>Information</span></a></li>\n");
      out.write("          </ul>\n");
      out.write("        </section>\n");
      out.write("        <!-- /.sidebar -->\n");
      out.write("      </aside>\n");
      out.write("\n");
      out.write("      <!-- Content Wrapper. Contains page content -->\n");
      out.write("      <div class=\"content-wrapper\">\n");
      out.write("        <!-- Content Header (Page header) -->\n");
      out.write("        <section class=\"content-header\">\n");
      out.write("          <h1>\n");
      out.write("            General Form Elements\n");
      out.write("            <small>Preview</small>\n");
      out.write("          </h1>\n");
      out.write("          <ol class=\"breadcrumb\">\n");
      out.write("            <li><a href=\"#\"><i class=\"fa fa-dashboard\"></i> Home</a></li>\n");
      out.write("            <li><a href=\"#\">Forms</a></li>\n");
      out.write("            <li class=\"active\">General Elements</li>\n");
      out.write("          </ol>\n");
      out.write("        </section>\n");
      out.write("\n");
      out.write("        <!-- Main content -->\n");
      out.write("        <section class=\"content\">\n");
      out.write("          <div class=\"row\">\n");
      out.write("            <!-- left column -->\n");
      out.write("            <div class=\"col-md-12\">\n");
      out.write("              <!-- general form elements -->\n");
      out.write("              <div class=\"box box-primary\">\n");
      out.write("                <div class=\"box-header with-border\">\n");
      out.write("                  <h3 class=\"box-title\">Work Order</h3>\n");
      out.write("                </div><!-- /.box-header -->\n");
      out.write("                <!-- form start -->\n");
      out.write("            <script lang=\"javascipt\">\n");
      out.write("            function GetXmlHttpObject()\n");
      out.write("            {\n");
      out.write("                var xmlHttp=null;\n");
      out.write("                try\n");
      out.write("                {\n");
      out.write("                 \n");
      out.write("                    xmlHttp=new XMLHttpRequest();\n");
      out.write("                }\n");
      out.write("                catch(e)\n");
      out.write("                {\n");
      out.write("                    try\n");
      out.write("                    {\n");
      out.write("                        xmlHttp=new ActiveXObject(\"Msxml2.XMLHTTP\");\n");
      out.write("                    }\n");
      out.write("                    catch(e)\n");
      out.write("                    {\n");
      out.write("                        xmlHttp=new ActiveXObject(\"Microsoft.XMLHTTP\");\n");
      out.write("                    }\n");
      out.write("                }\n");
      out.write("                return xmlHttp;\n");
      out.write("                \n");
      out.write("            }\n");
      out.write("            function getUserType()\n");
      out.write("            {\n");
      out.write("            xmlHttp=GetXmlHttpObject();\n");
      out.write("            if(xmlHttp==null)\n");
      out.write("            {\n");
      out.write("                alert(\"somethings wrong\");\n");
      out.write("                return;\n");
      out.write("            }\n");
      out.write("            var url=\"vendordd.jsp\";\n");
      out.write("            xmlHttp.open(\"GET\",url,true);\n");
      out.write("            xmlHttp.onreadystatechange=displayUserType;\n");
      out.write("            xmlHttp.send(null);\n");
      out.write("            }\n");
      out.write("            \n");
      out.write("            function displayUserType()\n");
      out.write("            {\n");
      out.write("                alert(\"xmlhttp.readystate:\"+xmlHttp.readyState);\n");
      out.write("                 \n");
      out.write("                if(xmlHttp.readyState==4 || xmlHttp.readyState==\"complete\")\n");
      out.write("                {\n");
      out.write("                    alert(\"in ready state 4\");\n");
      out.write("                    if(xmlHttp.status==200)\n");
      out.write("                    {\n");
      out.write("                        alert(\"xmlHttp.status:\"+xmlHttp.status);\n");
      out.write("                        var newStatus=xmlHttp.responseText;\n");
      out.write("                        alert(\"new status:\"+newStatus);\n");
      out.write("                    newStatus=newStatus.substring(newStatus.indexOf(\"[\")+1,newStatus.indexOf(\"]\"));    \n");
      out.write("                    var data=newStatus.split(\",\");\n");
      out.write("                    var optn=document.getElementById(\"usertypeid\");\n");
      out.write("                    for(var i=0;i<data.length;i++)\n");
      out.write("                        {\n");
      out.write("                            var id=data[i].substring(0,data[i].indexOf(\":\"));\n");
      out.write("                            var name=data[i].substring(data[i].indexOf(\":\")+1);\n");
      out.write("                            optn[i]=new Option(name,id);\n");
      out.write("                        }\n");
      out.write("                    }\n");
      out.write("                }\n");
      out.write("                else\n");
      out.write("                    {\n");
      out.write("                        alert(\"error:\"+xmlHttp.status);\n");
      out.write("                    }\n");
      out.write("            }\n");
      out.write("            \n");
      out.write("            function getLocation()\n");
      out.write("            {\n");
      out.write("            var id=document.getElementById(\"usertypeid\").value;\n");
      out.write("            xmlHttp=GetXmlHttpObject();\n");
      out.write("            if(xmlHttp==null)\n");
      out.write("            {\n");
      out.write("                alert(\"somethings wrong\");\n");
      out.write("                return;\n");
      out.write("            }\n");
      out.write("            var url=\"locationdd.jsp?id=\"+id;\n");
      out.write("            xmlHttp.open(\"GET\",url,true);\n");
      out.write("            xmlHttp.onreadystatechange=displayLocation;\n");
      out.write("            xmlHttp.send(null);\n");
      out.write("            }\n");
      out.write("            \n");
      out.write("            function displayLocation()\n");
      out.write("            {\n");
      out.write("                alert(\"xmlhttp.readystate:\"+xmlHttp.readyState);\n");
      out.write("                 \n");
      out.write("                if(xmlHttp.readyState==4 || xmlHttp.readyState==\"complete\")\n");
      out.write("                {\n");
      out.write("                    alert(\"in ready state 4\");\n");
      out.write("                    if(xmlHttp.status==200)\n");
      out.write("                    {\n");
      out.write("                        alert(\"xmlHttp.status:\"+xmlHttp.status);\n");
      out.write("                        var newStatus=xmlHttp.responseText;\n");
      out.write("                        alert(\"new status:\"+newStatus);\n");
      out.write("                    newStatus=newStatus.substring(newStatus.indexOf(\"[\")+1,newStatus.indexOf(\"]\"));    \n");
      out.write("                    var data=newStatus.split(\",\");\n");
      out.write("                    var optn=document.getElementById(\"location\");\n");
      out.write("                    for(var i=0;i<data.length;i++)\n");
      out.write("                        {\n");
      out.write("                            var id=data[i].substring(0,data[i].indexOf(\":\"));\n");
      out.write("                            var name=data[i].substring(data[i].indexOf(\":\")+1);\n");
      out.write("                            optn[i]=new Option(name,id);\n");
      out.write("                        }\n");
      out.write("                    }\n");
      out.write("                }\n");
      out.write("                else\n");
      out.write("                    {\n");
      out.write("                        alert(\"error:\"+xmlHttp.status);\n");
      out.write("                    }\n");
      out.write("            }\n");
      out.write("            \n");
      out.write("        \n");
      out.write("            </script>\n");
      out.write("                <form role=\"form\" action=\"woservlet\">\n");
      out.write("                  <div class=\"box-body\">\n");
      out.write("                    \n");
      out.write("                       <div class=\"form-group\">\n");
      out.write("                          <label for=\"exampleInputEmail1\">Work Order No.</label>\n");
      out.write("                      <label >");
      out.print(workkey);
      out.write("\n");
      out.write("                        \n");
      out.write("                      </label>\n");
      out.write("                       <input type=\"hidden\" name=\"key\" value=\"");
      out.print(workkey);
      out.write("\">\n");
      out.write("                      </div>\n");
      out.write("                      <div class=\"form-group\">\n");
      out.write("                          <label for=\"exampleInputEmail1\"> Vendor </label>\n");
      out.write("                      <select name=\"usertypeid\" id=\"usertypeid\">\n");
      out.write("                        \n");
      out.write("                      </select>\n");
      out.write("                      </div>\n");
      out.write("                      <div class=\"form-group\">\n");
      out.write("                          <label for=\"exampleInputEmail1\"> </label>\n");
      out.write("                      <!--<select name=\"location\" id=\"location\"  >\n");
      out.write("                        \n");
      out.write("                      </select>-->\n");
      out.write("                      </div>\n");
      out.write("                      <div class=\"box\">\n");
      out.write("                <div class=\"box-header\">\n");
      out.write("                  <h3 class=\"box-title\">Data Table With Full Features</h3>\n");
      out.write("                </div><!-- /.box-header -->\n");
      out.write("                <div class=\"box-body\">\n");
      out.write("                  <table id=\"item\" class=\"table table-bordered table-striped\">\n");
      out.write("                      \n");
      out.write("                          \n");
      out.write("                  </table>\n");
      out.write("                </div>\n");
      out.write("                </div>\n");
      out.write("                </div><!-- /.box-body -->\n");
      out.write("\n");
      out.write("                  <div class=\"box-footer\">\n");
      out.write("                    <button type=\"submit\" class=\"btn btn-primary\">Submit</button>\n");
      out.write("                  </div>\n");
      out.write("                 <!--<div class=\"box-footer\">\n");
      out.write("                     <button type=\"button\" class=\"btn btn-primary\" onclick=\"CreateItemTable();\">Add Item</button>\n");
      out.write("                  </div>\n");
      out.write("                </form>\n");
      out.write("              </div> box -->\n");
      out.write("\n");
      out.write("              <!-- Form Element sizes -->\n");
      out.write("             \n");
      out.write("\n");
      out.write("             \n");
      out.write("\n");
      out.write("              <!-- Input addon -->\n");
      out.write("             \n");
      out.write("\n");
      out.write("            </div><!--/.col (left) -->\n");
      out.write("            <!-- right column -->\n");
      out.write("            \n");
      out.write("          </div>   <!-- /.row -->\n");
      out.write("        </section><!-- /.content -->\n");
      out.write("      </div><!-- /.content-wrapper -->\n");
      out.write("        ");
      org.apache.jasper.runtime.JspRuntimeLibrary.include(request, response, "footercss.jsp", out, false);
      out.write("\n");
      out.write("       <!-- Control Sidebar -->\n");
      out.write("      <aside class=\"control-sidebar control-sidebar-dark\">\n");
      out.write("        <!-- Create the tabs -->\n");
      out.write("        <ul class=\"nav nav-tabs nav-justified control-sidebar-tabs\">\n");
      out.write("          <li><a href=\"#control-sidebar-home-tab\" data-toggle=\"tab\"><i class=\"fa fa-home\"></i></a></li>\n");
      out.write("          <li><a href=\"#control-sidebar-settings-tab\" data-toggle=\"tab\"><i class=\"fa fa-gears\"></i></a></li>\n");
      out.write("        </ul>\n");
      out.write("        <!-- Tab panes -->\n");
      out.write("        <div class=\"tab-content\">\n");
      out.write("          <!-- Home tab content -->\n");
      out.write("          <div class=\"tab-pane\" id=\"control-sidebar-home-tab\">\n");
      out.write("            <h3 class=\"control-sidebar-heading\">Recent Activity</h3>\n");
      out.write("            <ul class=\"control-sidebar-menu\">\n");
      out.write("              <li>\n");
      out.write("                <a href=\"javascript::;\">\n");
      out.write("                  <i class=\"menu-icon fa fa-birthday-cake bg-red\"></i>\n");
      out.write("                  <div class=\"menu-info\">\n");
      out.write("                    <h4 class=\"control-sidebar-subheading\">Langdon's Birthday</h4>\n");
      out.write("                    <p>Will be 23 on April 24th</p>\n");
      out.write("                  </div>\n");
      out.write("                </a>\n");
      out.write("              </li>\n");
      out.write("              <li>\n");
      out.write("                <a href=\"javascript::;\">\n");
      out.write("                  <i class=\"menu-icon fa fa-user bg-yellow\"></i>\n");
      out.write("                  <div class=\"menu-info\">\n");
      out.write("                    <h4 class=\"control-sidebar-subheading\">Frodo Updated His Profile</h4>\n");
      out.write("                    <p>New phone +1(800)555-1234</p>\n");
      out.write("                  </div>\n");
      out.write("                </a>\n");
      out.write("              </li>\n");
      out.write("              <li>\n");
      out.write("                <a href=\"javascript::;\">\n");
      out.write("                  <i class=\"menu-icon fa fa-envelope-o bg-light-blue\"></i>\n");
      out.write("                  <div class=\"menu-info\">\n");
      out.write("                    <h4 class=\"control-sidebar-subheading\">Nora Joined Mailing List</h4>\n");
      out.write("                    <p>nora@example.com</p>\n");
      out.write("                  </div>\n");
      out.write("                </a>\n");
      out.write("              </li>\n");
      out.write("              <li>\n");
      out.write("                <a href=\"javascript::;\">\n");
      out.write("                  <i class=\"menu-icon fa fa-file-code-o bg-green\"></i>\n");
      out.write("                  <div class=\"menu-info\">\n");
      out.write("                    <h4 class=\"control-sidebar-subheading\">Cron Job 254 Executed</h4>\n");
      out.write("                    <p>Execution time 5 seconds</p>\n");
      out.write("                  </div>\n");
      out.write("                </a>\n");
      out.write("              </li>\n");
      out.write("            </ul><!-- /.control-sidebar-menu -->\n");
      out.write("\n");
      out.write("            <h3 class=\"control-sidebar-heading\">Tasks Progress</h3>\n");
      out.write("            <ul class=\"control-sidebar-menu\">\n");
      out.write("              <li>\n");
      out.write("                <a href=\"javascript::;\">\n");
      out.write("                  <h4 class=\"control-sidebar-subheading\">\n");
      out.write("                    Custom Template Design\n");
      out.write("                    <span class=\"label label-danger pull-right\">70%</span>\n");
      out.write("                  </h4>\n");
      out.write("                  <div class=\"progress progress-xxs\">\n");
      out.write("                    <div class=\"progress-bar progress-bar-danger\" style=\"width: 70%\"></div>\n");
      out.write("                  </div>\n");
      out.write("                </a>\n");
      out.write("              </li>\n");
      out.write("              <li>\n");
      out.write("                <a href=\"javascript::;\">\n");
      out.write("                  <h4 class=\"control-sidebar-subheading\">\n");
      out.write("                    Update Resume\n");
      out.write("                    <span class=\"label label-success pull-right\">95%</span>\n");
      out.write("                  </h4>\n");
      out.write("                  <div class=\"progress progress-xxs\">\n");
      out.write("                    <div class=\"progress-bar progress-bar-success\" style=\"width: 95%\"></div>\n");
      out.write("                  </div>\n");
      out.write("                </a>\n");
      out.write("              </li>\n");
      out.write("              <li>\n");
      out.write("                <a href=\"javascript::;\">\n");
      out.write("                  <h4 class=\"control-sidebar-subheading\">\n");
      out.write("                    Laravel Integration\n");
      out.write("                    <span class=\"label label-warning pull-right\">50%</span>\n");
      out.write("                  </h4>\n");
      out.write("                  <div class=\"progress progress-xxs\">\n");
      out.write("                    <div class=\"progress-bar progress-bar-warning\" style=\"width: 50%\"></div>\n");
      out.write("                  </div>\n");
      out.write("                </a>\n");
      out.write("              </li>\n");
      out.write("              <li>\n");
      out.write("                <a href=\"javascript::;\">\n");
      out.write("                  <h4 class=\"control-sidebar-subheading\">\n");
      out.write("                    Back End Framework\n");
      out.write("                    <span class=\"label label-primary pull-right\">68%</span>\n");
      out.write("                  </h4>\n");
      out.write("                  <div class=\"progress progress-xxs\">\n");
      out.write("                    <div class=\"progress-bar progress-bar-primary\" style=\"width: 68%\"></div>\n");
      out.write("                  </div>\n");
      out.write("                </a>\n");
      out.write("              </li>\n");
      out.write("            </ul><!-- /.control-sidebar-menu -->\n");
      out.write("\n");
      out.write("          </div><!-- /.tab-pane -->\n");
      out.write("          <!-- Stats tab content -->\n");
      out.write("          <div class=\"tab-pane\" id=\"control-sidebar-stats-tab\">Stats Tab Content</div><!-- /.tab-pane -->\n");
      out.write("          <!-- Settings tab content -->\n");
      out.write("          <div class=\"tab-pane\" id=\"control-sidebar-settings-tab\">\n");
      out.write("            <form method=\"post\">\n");
      out.write("              <h3 class=\"control-sidebar-heading\">General Settings</h3>\n");
      out.write("              <div class=\"form-group\">\n");
      out.write("                <label class=\"control-sidebar-subheading\">\n");
      out.write("                  Report panel usage\n");
      out.write("                  <input type=\"checkbox\" class=\"pull-right\" checked>\n");
      out.write("                </label>\n");
      out.write("                <p>\n");
      out.write("                  Some information about this general settings option\n");
      out.write("                </p>\n");
      out.write("              </div><!-- /.form-group -->\n");
      out.write("\n");
      out.write("              <div class=\"form-group\">\n");
      out.write("                <label class=\"control-sidebar-subheading\">\n");
      out.write("                  Allow mail redirect\n");
      out.write("                  <input type=\"checkbox\" class=\"pull-right\" checked>\n");
      out.write("                </label>\n");
      out.write("                <p>\n");
      out.write("                  Other sets of options are available\n");
      out.write("                </p>\n");
      out.write("              </div><!-- /.form-group -->\n");
      out.write("\n");
      out.write("              <div class=\"form-group\">\n");
      out.write("                <label class=\"control-sidebar-subheading\">\n");
      out.write("                  Expose author name in posts\n");
      out.write("                  <input type=\"checkbox\" class=\"pull-right\" checked>\n");
      out.write("                </label>\n");
      out.write("                <p>\n");
      out.write("                  Allow the user to show his name in blog posts\n");
      out.write("                </p>\n");
      out.write("              </div><!-- /.form-group -->\n");
      out.write("\n");
      out.write("              <h3 class=\"control-sidebar-heading\">Chat Settings</h3>\n");
      out.write("\n");
      out.write("              <div class=\"form-group\">\n");
      out.write("                <label class=\"control-sidebar-subheading\">\n");
      out.write("                  Show me as online\n");
      out.write("                  <input type=\"checkbox\" class=\"pull-right\" checked>\n");
      out.write("                </label>\n");
      out.write("              </div><!-- /.form-group -->\n");
      out.write("\n");
      out.write("              <div class=\"form-group\">\n");
      out.write("                <label class=\"control-sidebar-subheading\">\n");
      out.write("                  Turn off notifications\n");
      out.write("                  <input type=\"checkbox\" class=\"pull-right\">\n");
      out.write("                </label>\n");
      out.write("              </div><!-- /.form-group -->\n");
      out.write("\n");
      out.write("              <div class=\"form-group\">\n");
      out.write("                <label class=\"control-sidebar-subheading\">\n");
      out.write("                  Delete chat history\n");
      out.write("                  <a href=\"javascript::;\" class=\"text-red pull-right\"><i class=\"fa fa-trash-o\"></i></a>\n");
      out.write("                </label>\n");
      out.write("              </div><!-- /.form-group -->\n");
      out.write("            </form>\n");
      out.write("          </div><!-- /.tab-pane -->\n");
      out.write("        </div>\n");
      out.write("      </aside><!-- /.control-sidebar -->\n");
      out.write("      <!-- Add the sidebar's background. This div must be placed\n");
      out.write("           immediately after the control sidebar -->\n");
      out.write("      <div class=\"control-sidebar-bg\"></div>\n");
      out.write("    </div><!-- ./wrapper -->\n");
      out.write("\n");
      out.write("    <!-- jQuery 2.1.4 -->\n");
      out.write("    <script src=\"admincss/plugins/jQuery/jQuery-2.1.4.min.js\"></script>\n");
      out.write("    <!-- Bootstrap 3.3.5 -->\n");
      out.write("    <script src=\"admincss/bootstrap/js/bootstrap.min.js\"></script>\n");
      out.write("    <!-- FastClick -->\n");
      out.write("    <script src=\"admincss/plugins/fastclick/fastclick.min.js\"></script>\n");
      out.write("    <!-- AdminLTE App -->\n");
      out.write("    <script src=\"admincss/dist/js/app.min.js\"></script>\n");
      out.write("    <!-- AdminLTE for demo purposes -->\n");
      out.write("    <script src=\"admincss/dist/js/demo.js\"></script>\n");
      out.write("  </body>\n");
      out.write("</html>\n");
    } catch (Throwable t) {
      if (!(t instanceof SkipPageException)){
        out = _jspx_out;
        if (out != null && out.getBufferSize() != 0)
          out.clearBuffer();
        if (_jspx_page_context != null) _jspx_page_context.handlePageException(t);
        else throw new ServletException(t);
      }
    } finally {
      _jspxFactory.releasePageContext(_jspx_page_context);
    }
  }
}
